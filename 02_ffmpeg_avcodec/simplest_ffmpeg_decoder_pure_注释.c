...
int main(int argc, char* argv[])
{
	...
	// 根据宏选择输入的文件格式，这里可以改为下拉菜单包含所有可解码的格式，用户选择
	// tbtn_avcodec_enum通过读入名字构建下拉菜单，然后avcodec_find_decoder_by_name()获取AVCodec
	// 加一个QFileDialog打开文件，对应act_file_open
#if TEST_HEVC
	enum AVCodecID codec_id=AV_CODEC_ID_HEVC; // 同AV_CODEC_ID_H265
	...
	// 注册编解码器，这里不用ac_register_all()原因是该函数中注册包含了解复用器等，本例不需要
	...
	// 为结构体AVCodecContext分配内存并设置默认参数，结束后对应avcodec_free_context释放内存
    pCodecCtx = avcodec_alloc_context3(pCodec);
    if (!pCodecCtx){
        printf("Could not allocate video codec context\n");
        return -1;
    }
	// 创建AVCodecParserContext结构体
